...
export default async function GenrePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: genre }, { data: tracks }] = await Promise.all([
    supabase.from('genres').select('*').eq('id', id).single(),
    supabase.from('tracks_view').select('*').eq('genre_id', id).order('created_at', { ascending: false }),
  ]);
  ...
  return (
    <div className="p-5 md:p-8 animate-fade-in">
      ...
      <div className="mb-8">
        <p className="text-[10px] font-mono text-dim uppercase tracking-widest mb-1">Genre</p>
        <h1 className="font-display text-3xl font-bold text-bright">{genre.name}</h1>
